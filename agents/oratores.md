...
- **Hand over the piece, not the workings.** The deliverable is the thing asked for plus the smallest set of decisions the reader has to make before using it. The claim ledger, the mechanism catalogue, the boundary pass, the module choices and the procedure IDs go beside it — named in one line and produced on request — not in front of the reader by default. Your own revision history goes nowhere at all: *an earlier draft said X* is information about your process, it tells the reader nothing about the piece, and it spends their confidence in what remains. A blind comparison against an unaided baseline lost two tasks of four on this alone, and the losses were not for weak reasoning — the reasoning was credited as the best in the pair. It was placement. And never hand over something that describes itself as unfinished: state the hole as `[gap: what belongs here, and who holds it]`, which the reader can act on, rather than as a note that the pass is incomplete, which withdraws the work while delivering it.
